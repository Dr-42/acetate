...
void ac_darray_set_resize_factor(float factor) {
    AC_DARRAY_RESIZE_FACTOR = factor;
}
...
void ac_darray_push(ac_darray_t* darray, void* element) {
    if (darray->size == darray->capacity) {
        darray->capacity =
            (uint64_t)ceilf((float)darray->capacity * AC_DARRAY_RESIZE_FACTOR);
        darray->data =
            ac_realloc(darray->data, darray->element_size * darray->capacity,
                       AC_MEM_ENTRY_DS);
    }
    ac_memcpy((uint8_t*)darray->data + darray->size * darray->element_size,
              element, darray->element_size);
    darray->size++;
}
...
void ac_darray_pop(ac_darray_t* darray, void* element) {
    ac_memcpy(
        element,
        (uint8_t*)darray->data + (darray->size - 1) * darray->element_size,
        darray->element_size);
    darray->size--;
}
...
void ac_darray_insert(ac_darray_t* darray, uint64_t index, void* element) {
    ...
    ac_memmove((uint8_t*)darray->data + (index + 1) * darray->element_size,
               (uint8_t*)darray->data + index * darray->element_size,
               (darray->size - index) * darray->element_size);
    ac_memcpy((uint8_t*)darray->data + index * darray->element_size, element,
              darray->element_size);
    darray->size++;
}

void ac_darray_remove(ac_darray_t* darray, uint64_t index, void* element) {
    ac_memcpy(element, (uint8_t*)darray->data + index * darray->element_size,
              darray->element_size);
    ac_memmove((uint8_t*)darray->data + index * darray->element_size,
               (uint8_t*)darray->data + (index + 1) * darray->element_size,
